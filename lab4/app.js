...
import { getAllTeams, addTeam, getTeamsByID } from "./teams.js";
...
else if(pathname.startWith("/ap1/v1/teams") && method === "PUT")
{const id = Number(pathname.split("/").pop());
    const oldTeam = getTeamsByID(id);
    if(!oldTeam)
        return sendJson(res,400, {error: `teams with id: ${id} not found`});
    const {tname,tl, members} =  await parseJSONBody(req);
    if(!tname || !tl || members)
        return sendJson(res, 400, {
    error: "team name, team leader, or memeber not defined"});
    const update = updatTeam(id, {tname, tl, memebers});
    return sendJson(
        res,
        200,
        update,
